...
class TwoPaneApp(App):
    ...
    def load_aws_accounts(self)->None:
        ...
        for profile in config.sections():
            ...
            if config[profile].get('sso_session', None):
                self.data[nice_name] = f"Aws account ID: {config[profile].get('sso_account_id', 'broken')}"
    ...
    def retrieve_creds(self, account_name:str)-> bool:
        ...
        output = subprocess.run(f"aws sso login --profile {account_name}", shell=True,  capture_output=True)
    ...
    def compose(self) -> ComposeResult:
        with Vertical():
            ...
            yield Button("Get AWS credentials", id="my_button")
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if event.button.id == "my_button":
            ...
            info_pane = self.query_one("#info_pane", Static)
            info_pane.update(f"Getting SSO credentials for {selected_account}, this will launch a browser window")
